...
def run_gate4_rerun(source_asset_id):
    print(f"=== STARTING GATE 4 RERUN: Miaomiao -> Lustify Img2Img ===")
    print(f"Source Asset: {source_asset_id}")
    store = MissionStore()
    
    golden_id = "ada_alpha_golden_1_0_3916a42f"
    mission_id = f"{golden_id}_gate4_rerun2"
    ...
    if mission and mission.status in ["COMPLETED", "COMPLETE", "FINAL"]:
        ...
        print(f"Gate 4 Rerun finished with status: {mission.status}")
    
    if mission.status not in ["COMPLETED", "COMPLETE", "FINAL"]:
        print(f"GATE 4 RERUN FAILED! Status={mission.status}, Error={getattr(mission, 'error_message', '')}")
        sys.exit(1)
        
    lib = AssetLibrary()
    assets = lib.get_assets(mission_id=mission.mission_id)
    print(f"Assets found in library for Gate 4 Rerun: {len(assets)}")
    
    if len(assets) == 0:
        print("GATE 4 RERUN FAILED! No assets found in library.")
        sys.exit(1)
        
    print("GATE 4 RERUN PASS (Reinterpretation completed).")
    return mission.mission_id, golden_id

if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python golden_gate4_rerun.py <source_asset_id>")
        sys.exit(1)
    run_gate4_rerun(sys.argv[1])
